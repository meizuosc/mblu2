...
//        UINT_16 u2Etype = 0;
	...
	if (u2Etype == ETH_P_ARP) {
		...
		int opCode = (pc[ETH_TYPE_LEN_OFFSET + 8] << 8) | (pc[ETH_TYPE_LEN_OFFSET + 8 + 1]);
		/* DBGLOG(INIT, WARN, ("get arp\n"));
		DBGLOG(INIT, WARN, ("##########opCode is %d\n",opCode)); */
		if (opCode == 0x0002) {
			arpMoniter = 0;
                        /* DBGLOG(INIT, WARN, ("get arp response!!!!!\n")); */
                        if (prBssInfo->prStaRecOfAP->aucMacAddr){
	                	/* DBGLOG(INIT, WARN, ("########prAdapter->prAisBssInfo->prStaRecOfAP->aucMacAddr is , %x:%x:%x:%x:%x:%x.\n",
        				...
        				prBssInfo->prStaRecOfAP->aucMacAddr[4], prBssInfo->prStaRecOfAP->aucMacAddr[5])); */
				...
				if(prBssInfo && prBssInfo->prStaRecOfAP && prBssInfo->prStaRecOfAP->aucMacAddr) {
					if(EQUAL_MAC_ADDR(&(pc[ETH_TYPE_LEN_OFFSET + 10]), prBssInfo->prStaRecOfAP->aucMacAddr)) {
						strncpy(apIp, &(pc[ETH_TYPE_LEN_OFFSET + 16]), 4);
						/* DBGLOG(INIT, WARN, ("get arp response from AP %d.%d.%d.%d\n",
