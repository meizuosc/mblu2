...
#if (ARP_MONITER_ENABLE==1)
	if ((u2EtherTypeLen == ETH_P_ARP) && (!strncmp(apIp, &aucLookAheadBuf[ETH_TYPE_LEN_OFFSET + 26], 4)))
	{
		UINT_8 *pucDstIp = &aucLookAheadBuf[ETH_HLEN];
		if (pucDstIp[7] == ARP_PRO_REQ)
		{
			/* DBGLOG(INIT, WARN, ("<tx> OS tx a arp req to %d.%d.%d.%d\n",
		...
		{
			DBGLOG(INIT, WARN, ("<tx> OS tx a arp rsp to %d.%d.%d.%d\n",
