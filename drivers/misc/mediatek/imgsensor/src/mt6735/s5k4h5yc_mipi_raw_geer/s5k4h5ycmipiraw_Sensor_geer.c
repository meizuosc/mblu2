...
#include "s5k4h5ycmipiraw_Sensor_geer.h"

#define PFX "S5K4H5YC_camera_sensor_geer"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H5YC_SENSOR_ID_GEER,
...
{              LOG_INF("E//VGA 120fps");
			...
			write_cmos_sensor(0x0349,0xD1);     	//x_addr_end L //liukun mirror&flip slowmotion dark CF->D1                                                      
...
static kal_uint16 S5K4h5yc_MIPI_read_otp_vendor_id(void)
{
    ...
    {  
      ...
      case 0x10:
       ...
       break;
      default:	 
      LOG_INF("jiacheng S5K4H5_Sensor: info_flag error value: %d \n ",info_flag);
      break;
    }
	write_cmos_sensor(0x3a00, 0x00); //otp disable read
	LOG_INF("jiacheng ModuleHouseID = %x \n", ModuleHouseID);
	return ModuleHouseID;
}

static void S5K4h5yc_MIPI_read_otp_wb(void)
{
	int i=0;
	kal_uint16 SN = 0;
   	kal_uint16 golden_R = 0, 
			   ...
			   LOG_INF("jiacheng %s,wb_flag = 0x%x\n",__func__,wb_flag);
			   switch (wb_flag)
			  {  
				case 0x40:
				...
				break;
				default:	 
				LOG_INF("jiacheng S5K4H5_Sensor: wb_flag error value: %d \n ",wb_flag);
				break;
			  }
			  printk("[jiacheng]---s5k4h5yc--SN-----number---begin--\n");
			  for(i=0x3A1E;i<0x3A2D;i++){
				SN = read_cmos_sensor(i);
		        printk("%d",SN);
				if(i==0x3A2C)
				  printk("\n[jiacheng]---s5k4h5yc--SN-----number--end---\n");
			  }
			  
			  write_cmos_sensor(0x3a00, 0x00); //otp disable read
  		LOG_INF("jiacheng [S5K4h5yc_MIPI_read_otp_wb] current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
   		...
   		LOG_INF("jiacheng [S5K4h5yc_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", r_ratio, b_ratio);      
}
static void S5K4h5yc_MIPI_algorithm_otp_wb1()
{
   kal_uint16 R_GAIN = 0, 
   	          B_GAIN = 0,
   	          Gr_GAIN = 0, 
   ...
   LOG_INF("[S5K4h5yc] [S5K4h5yc_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN, G_GAIN);    
}
static void S5K4h5yc_MIPI_update_wb_register_from_otp(void)
{   
   S5K4h5yc_MIPI_read_otp_wb();
   S5K4h5yc_MIPI_algorithm_otp_wb1();
}
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			#if defined(S5K4h5yc_USE_AWB_OTP)						
			 if (S5K4h5yc_MIPI_read_otp_vendor_id() == 0x09)
				{
				return ERROR_NONE;
				} else {
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			    ...
			    sensor_id = 0xFFFFFFFF;
              #endif
			if ((sensor_id == imgsensor_info.sensor_id) && (S5K4h5yc_MIPI_read_otp_vendor_id() == 0x09)) {				
