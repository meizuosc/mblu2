...
#endif
//Flyme_EDIT ldx@meizu.com modify for longexp
#define MAX_EXP_LINES 951638 //for capture mode 10s

static kal_uint32 S5K3L2XX_capture_shutter_geer = 1;
static kal_uint32 S5K3L2XX_longexp_linelength_geer = 4512;
static kal_uint16 S5K3L2XX_capture_gain_geer = 64;
static kal_uint8 S5K3L2XX_long_exp_flag_geer = 0;
//modify end
...
{
	kal_uint16 line_length = 0, framerate = 0 , frame_length_min = 0, read_register0005_value = 0;
	kal_uint32 pixelclock = 0, frame_length = 0;
...
//Flyme_EDIT ldx@meizu.com add for longexp
...
//Flyme_EDIT ldx@meizu.com add for longexp
       if(shutter >= MAX_EXP_LINES && IMGSENSOR_MODE_CAPTURE == imgsensor.sensor_mode) {
            line_length = 0xFFFF;
       } else if(shutter > (imgsensor_info.max_frame_length - SHUTTER_FRAMELENGTH_MARGIN)) {
            line_length = S5K3L2XX_longexp_linelength_geer;
	}
	else {
...
//Flyme_EDIT ldx@meizu.com modify for longexp
	...
	if(read_register0005_value == 0xff)
		LOG_INF("[error][0x0005, 0xff]sensor not output\n");
#else
       if(shutter == 4 && IMGSENSOR_MODE_CAPTURE == imgsensor.sensor_mode) {
            LOG_INF("long exp restore");
            S5K3L2XX_long_exp_flag_geer = 0;
            ...
            write_cmos_sensor(0x0101, 0x03);//flip and mirror

            write_cmos_sensor1(0x0340, frame_length);
            write_cmos_sensor1(0x0202, (UINT16)shutter);
            write_cmos_sensor1(0x0342, line_length);
       ...
       if(shutter <= (imgsensor_info.max_frame_length - SHUTTER_FRAMELENGTH_MARGIN)){
		write_cmos_sensor(0x0104, 0x01);	
		write_cmos_sensor1(0x0340, frame_length);
		write_cmos_sensor1(0x0202, (UINT16)shutter);
              write_cmos_sensor1(0x0342, line_length); 
	...
	else{           /*long shutter setting*/
            write_cmos_sensor(0x0104, 0x01);
            write_cmos_sensor1(0x0340, 0xFFFF);
            write_cmos_sensor1(0x0202, 0xFFEF);
            write_cmos_sensor1(0x0342, line_length);
            write_cmos_sensor(0x0104, 0x00);
       }

       if(!S5K3L2XX_long_exp_flag_geer) {
        	read_register0005_value = read_cmos_sensor(0x0005);
        	if(read_register0005_value == 0xff)
        		LOG_INF("[error][0x0005, 0xff]sensor not output\n");
       }
...
static void longexp_setting(kal_uint32 shutter,kal_uint16 gain)
{
    ...
    if(shutter <= (imgsensor_info.max_frame_length - imgsensor_info.margin)){
        ...
        write_cmos_sensor(0x0104, 0x01);
        write_cmos_sensor1(0x0340, 0xFFFF); //frame_length
        write_cmos_sensor1(0x0202, 0xFFEF); //shutter
        write_cmos_sensor1(0x0342, line_length); //line_length
        write_cmos_sensor(0x0104, 0x00);
    }
...
//Flyme_EDIT ldx@meizu.com modify for longexp
    if(S5K3L2XX_long_exp_flag_geer) {
        longexp_setting(S5K3L2XX_capture_shutter_geer, S5K3L2XX_capture_gain_geer);
    } else {
...
//Flyme_EDIT ldx@meizu.com modify for longexp
    S5K3L2XX_long_exp_flag_geer = 0;
    S5K3L2XX_capture_shutter_geer = 1;
    S5K3L2XX_longexp_linelength_geer = 4512;
    S5K3L2XX_capture_gain_geer = 64;
//add end
    spin_unlock(&imgsensor_drv_lock);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_LONGEXP_PARAM:
            S5K3L2XX_capture_shutter_geer = *feature_data;
            S5K3L2XX_longexp_linelength_geer = *(feature_data + 1);
            S5K3L2XX_capture_gain_geer = (*(feature_data + 2)) & 0xffff;
            S5K3L2XX_long_exp_flag_geer = 1;
            LOG_INF("SENSOR_FEATURE_SET_LONGEXP_PARAM shutter=%d, linelength = %d,Gain=%d\n",
                S5K3L2XX_capture_shutter_geer,S5K3L2XX_longexp_linelength_geer,S5K3L2XX_capture_gain_geer);
            break;
