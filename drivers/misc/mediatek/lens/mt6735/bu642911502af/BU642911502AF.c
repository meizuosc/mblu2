...
#define AF_DRVNAME "BU642911502AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x18
#define PLATFORM_DRIVER_NAME "lens_actuator_BU642911502AF"
#define AF_DRIVER_CLASS_NAME "actuatordrv_BU642911502AF"

static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO(AF_DRVNAME, 0x18)};
...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
    ...
    char pBuff[2];
	char puSend =0xf4;
	i2c_master_send(g_pstAF_I2Cclient, &puSend, 1);
    i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, pBuff , 2);
printk("[jiacheng]-----read--pBuff[0]=%d,pBuff[1]=%d\n",pBuff[0],pBuff[1]);
    if (i4RetValue < 0)
    {
    ...
    *a_pu2Result = (((u16)pBuff[0]) << 8) && 0x300 + (pBuff[1]);
	printk("[jiacheng]-----read--a_pu2Result=%d\n",*a_pu2Result);
...
static int s4AF_WriteReg(u16 a_u2Data)
{
    ...
    char puSendCmd[2] = {(char)(a_u2Data>>8)+0xf4 , (char)(a_u2Data & 0xFF)};	//(1)0xc0, (2)0xf4:direct mode 

    //LOG_INF("g_sr %d, write %d \n", g_sr, a_u2Data);
    g_pstAF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
    i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
	printk("[jiacheng]-----send--value=%d\n",a_u2Data);
    if (i4RetValue < 0)
    {
...
inline static int getAFInfo(__user stBU642911502AF_MotorInfo * pstMotorInfo)
{
    stBU642911502AF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stBU642911502AF_MotorInfo)))
    {
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case BU642911502AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stBU642911502AF_MotorInfo *)(a_u4Param));
        break;
        case BU642911502AFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;

        case BU642911502AFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case BU642911502AFIOC_T_SETMACROPOS :
...
static int AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //g_pstAF_I2Cclient->addr = I2C_REGISTER_ID;//liukun@wind-mobi.com 20150103
...
static int __init BU642911502AF_i2C_init(void)
{
...
static void __exit BU642911502AF_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

module_init(BU642911502AF_i2C_init);
module_exit(BU642911502AF_i2C_exit);

MODULE_DESCRIPTION("BU642911502AF lens module driver");
