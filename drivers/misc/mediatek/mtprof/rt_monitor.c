...
void setup_mt_rt_mon_info(struct task_struct *p)
{
	...
	if (!find) {
		...
		if (!mtmon) {
			pr_err("mtmon unable to kmalloc\n");
			return;
		}
...
void stop_rt_mon_task(void)
{
	...
	rt_dur_ts = cputime_sub(rt_end_ts, rt_start_ts);
	...
	list_for_each_entry(tmp, &mt_rt_mon_head.list, list) {
		...
		if (tsk != NULL) {
			...
			tmp->isr_time = cputime_sub(tsk->se.mtk_isr_time, tmp->isr_time);
			strcpy(tmp->comm, tsk->comm);
		}

		if (tmp->cputime >= (tmp->cputime_init + tmp->isr_time)) {
			cost_cputime =
			    cputime_sub(tmp->cputime - tmp->isr_time, tmp->cputime_init);
...
void save_mt_rt_mon_info(struct task_struct *p, unsigned long long ts)
{
	...
	rt_dur_ts = cputime_sub(mon_now_ts, rt_start_ts);
	do_div(rt_dur_ts, 1000000);	/* put prof_dur_ts to ms */
	...
	mtmon = kmalloc(sizeof(struct mt_rt_mon_struct), GFP_KERNEL);
	if (!mtmon) {
		pr_err("mtmon unable to kmalloc\n");
		return;
	}
...
void end_mt_rt_mon_info(struct task_struct *p)
{
	...
	list_for_each_entry(tmp, &mt_rt_mon_head.list, list) {
		if (p->pid == tmp->pid) {
			...
			/* update cputime */
			tmp->cputime = p->se.sum_exec_runtime;
			tmp->isr_time = cputime_sub(p->se.mtk_isr_time, tmp->isr_time);
			find = 1;
			break;
		}
	}
...
static int __init mt_rt_mon_init(void)
{
	...
	spin_lock_irqsave(&mt_rt_mon_lock, irq_flags);
	INIT_LIST_HEAD(&(mt_rt_mon_head.list));
	mt_rt_mon_enabled = 0;
	rt_mon_count = 0;
	spin_unlock_irqrestore(&mt_rt_mon_lock, irq_flags);
	return 0;
}
device_initcall(mt_rt_mon_init);
