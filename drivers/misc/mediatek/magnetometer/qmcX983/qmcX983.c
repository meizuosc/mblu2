 ...
 */
 /*dixiaobing@wind-mobi.com 20151020 start*/
...
int count = 0;
int hw_d_old[3] = { 0 };
/*dixiaobing@wind-mobi.com 20151020 end*/
...
/*dixiaobing@wind-mobi.com 20151020 start*/
	MSE_LOG("Hx=%d, Hy=%d, Hz=%d , ICharging_mag=%d\n",hw_d[0],hw_d[1],hw_d[2],ICharging_mag);

	if(upmu_is_chr_det() == KAL_TRUE) 
	{
		if((count<150)&& (count % 20 == 0))
		{
			val = battery_meter_get_charging_current();
			hw_d[0] = hw_d_old[0];
			hw_d[2] = hw_d_old[2];
		}
		else if(count< 1000)///delay 500 count, 10ms 
		{
			hw_d[0] = hw_d[0] - 45*val/1000;
			hw_d[2] = hw_d[2] + 23*val/1000;
		}
		else
		{
			hw_d[0] = hw_d[0] - 45*ICharging_mag/1000;
			hw_d[2] = hw_d[2] + 23*ICharging_mag/1000;	
		}
