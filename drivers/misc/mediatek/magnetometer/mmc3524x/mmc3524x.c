...
#define MMC3524X_M_NEW_ARCH
#ifdef MMC3524X_M_NEW_ARCH
#include "mag.h"
#endif
/*----------------------------------------------------------------------------*/
//#define DEBUG 1
...
extern struct mag_hw* mmc3524_get_cust_mag_hw(void); 
...
#define MMCFUNC(func) MMCDBG(KERN_INFO "mmc3524x " func " is called\n")
#else
#define MMCFUNC(func)
#endif
...
static int ECS_GetRawData(int data[3])
{
	...
	if(err !=0 )
	{
		MMCDBG(KERN_ERR "MMC3524X_IOC_TM failed\n");
		return -1;
	}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if(NULL == obj)
	{
		MMCDBG(KERN_ERR "mmc3524x_i2c_data is null!!\n");
		return 0;
	}	
...
READ_ID_FAIL:
    MMCDBG(KERN_ERR"Auto Test: read id error!!");
    return -1;
RESET_DEV_FAIL:
    MMCDBG(KERN_ERR"Auto Test: reset device error!!");
    return -1;
READ_DATA_FAIL:
    MMCDBG(KERN_ERR"Auto Test: read data error!!");
    return -1;
}
...

{
	...
	switch (cmd)
	{
		...
		case MMC31XX_IOC_SET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MMCDBG(KERN_ERR "MMC3524X_IOC_SET failed\n");
				return -EFAULT;
			}
			...
			data[1] = MMC3524X_CTRL_SET;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG(KERN_ERR "MMC3524X_IOC_SET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3524X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG(KERN_ERR "MMC3524X_IOC_SET failed3\n");
				return -EFAULT;
			}
			msleep(MMC3524X_DELAY_SET);
			break;
		...
		case MMC31XX_IOC_RESET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MMCDBG(KERN_ERR "MMC3524X_IOC_RESET failed\n");
				return -EFAULT;
			}
			/* wait external capacitor charging done for next SET/RESET */
			msleep(MMC3524X_DELAY_RST);
			...
			data[1] = MMC3524X_CTRL_RESET;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG(KERN_ERR "MMC3524X_IOC_RESET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3524X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	               MMCDBG(KERN_ERR "MMC3524X_IOC_RESET failed3\n");
				return -EFAULT;
			}
			msleep(1);
			break;
		...
		case MMC31XX_IOC_READ:
			...
			if(I2C_RxData(data, 6) < 0)
			{
				MMCDBG(KERN_ERR "MMC3524X_IOC_READ failed\n");
				return -EFAULT;
			}
			vec[0] = data[1] << 8 | data[0];
			vec[1] = data[3] << 8 | data[2];
			...
			{
				MMCDBG(KERN_ERR "MMC3524X_IOC_READ: copy to user failed\n");
				return -EFAULT;
			}
			break;
		...
		case MMC31XX_IOC_READXYZ:
			...
			if(copy_to_user(argp, vec, sizeof(vec)))
			{
				MMCDBG(KERN_ERR "MMC3524X_IOC_READXYZ: copy to user failed\n");
				return -EFAULT;
			}
			break;			
		...
		case MSENSOR_IOCTL_SENSOR_ENABLE:
			...
			MMCDBG("<7>planar Read register No. 0x%0x\n", data[0]);
			reg_value = data[0];
		...
		case MMC3524X_IOC_WRITE_REG:
			...
			MMCDBG("<7>planar Write '0x%0x' to  register No. 0x%0x\n", data[0], data[1]);
		...
		if (copy_from_user(&data, argp, sizeof(data)))
			return -EFAULT;
		MMCDBG("<7> planar Read %d registers from 0x%0x\n", data[1], data[0]);	
...
static long mmc3524x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
		...
		{
			MMCDBG(KERN_ERR "file->f_op OR file->f_op->unlocked_ioctl is null!\n");
			return -ENOTTY;
		}
		...
		switch(cmd)
		{
			case COMPAT_MMC31XX_IOC_TM:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC31XX_IOC_TM is failed!\n");
				}
				break;
			...
			case COMPAT_MMC31XX_IOC_SET:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC31XX_IOC_SET is failed!\n");
				}
				break;
			...
			case COMPAT_MMC31XX_IOC_RM:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC31XX_IOC_RM is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_RESET:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC31XX_IOC_RESET is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_RRM:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC31XX_IOC_RRM is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_READ:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC31XX_IOC_READ is failed!\n");
				}
			...
			case COMPAT_MMC31XX_IOC_READXYZ:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC31XX_IOC_READXYZ is failed!\n");
				}
			...
			case COMPAT_MMC3524X_IOC_READ_REG:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC3524X_IOC_READ_REG is failed!\n");
				}		
				break;
			...
			case COMPAT_MMC3524X_IOC_WRITE_REG:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC3524X_IOC_WRITE_REG is failed!\n");
				}
			...
			case COMPAT_MMC3524X_IOC_READ_REGS:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MMC3524X_IOC_READ_REGS is failed!\n");
				}	
				break; 
			...
			case COMPAT_ECOMPASS_IOC_GET_DELAY:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_ECOMPASS_IOC_GET_DELAY is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_SET_YPR:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_YPR is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_OPEN_STATUS:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_ECOMPASS_IOC_GET_OPEN_STATUS is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_MFLAG:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_ECOMPASS_IOC_GET_MFLAG is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_OFLAG:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_ECOMPASS_IOC_GET_OFLAG is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_READ_CHIPINFO:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MSENSOR_IOCTL_READ_CHIPINFO is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_READ_SENSORDATA:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MSENSOR_IOCTL_READ_SENSORDATA is failed!\n");
				}
			...
			case COMPAT_ECOMPASS_IOC_GET_LAYOUT:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_ECOMPASS_IOC_GET_LAYOUT is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE is failed!\n");
				}
			...
			case COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
				...
				if(ret < 0)
				{
					MMCDBG(KERN_ERR "COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA is failed!\n");
				}
...
int mmc3524x_gyro_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	//MMCDBG("Kaka mmc3524x_gyro_operate\n");
	int err = 0;
	int value;
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
				vector_data = (hwm_sensor_data *)buff_out;
				mutex_lock(&sensor_data_mutex);
...
/*----------------------------------------------------------------------------*/
static int mmc3524x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{       MMCDBG( "rooney222222222222welcometomsensor\n");
	...
	data->hw = mmc3524_get_cust_mag_hw();	
...
//modified for mmc3524x----changhua start
	...
	if(I2C_TxData(tmp, 2) < 0)
	{
		MMCDBG(KERN_ERR "mmc3524x_device set ST cmd failed\n");
		goto exit_kfree;
	}
#endif
	...
	tmp[1] = MMC3524X_CTRL_REFILL;
	if (I2C_TxData(tmp, 2) < 0) {
		MMCDBG(KERN_ERR "mmc3416x_device I2C_TxData failed\n");
		err = -ENOMEM;
		goto exit_kfree;
	}
	msleep(MMC3524X_DELAY_SET);
...
MMCDBG("[znw]%d\n",__LINE__);	
	...
	{
		MMCDBG(KERN_ERR "mmc3524x_device register failed\n");
		goto exit_misc_device_register_failed;	
	}    
		MMCDBG( "rooney 1111 mmc3524x_device register ok\n");
...
//zhounengwen@wind-mobi.com gyro begin
...
//zhounengwen@wind-mobi.com gyro end
...
#endif
	msensor_name = "mmc3524x";	 //modify yudengwu 2015-01-30
...
#endif
	{
		MMCDBG(KERN_ERR "mmc3524x_delete_attr fail: %d\n", err);
	}
...
static int mmc3524x_local_init(void)
{
	struct mag_hw *hw = mmc3524_get_cust_mag_hw();
...
/*----------------------------------------------------------------------------*/
static int mmc_probe(struct platform_device *pdev) 
{        MMCDBG( "rooney1111111111welcometomsensor\n");
	struct mag_hw *hw = mmc3524_get_cust_mag_hw();
