...
/*oujiacheng@wind-mobi.com 2015.5.22 add begin*/
    //power ON
    if (On) {
        ...
        if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW, currSensorName)) && (pinSetIdx == 0))
        {
			PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW \n");
        ...
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW_GEER, currSensorName)) && (pinSetIdx == 0))
        {
			PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW \n");
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
        ...
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2355_MIPI_RAW, currSensorName)) && (pinSetIdx == 1))
         {
        	PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_GC2355_MIPI_RAW \n");
			...
			//DVDD VGP3
			mt_set_gpio_mode(GPIO_CAMERA_LDO_SUB_VCAMD,GPIO_CAMERA_LDO_SUB_VCAMD_M_GPIO);
			mt_set_gpio_dir(GPIO_CAMERA_LDO_SUB_VCAMD,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO_CAMERA_LDO_SUB_VCAMD,GPIO_OUT_ONE);
            mdelay(3);	
			
	        //VCAM_A
			#if 0
			mt_set_gpio_mode(GPIO_CAMERA_LDO_SUB_VCAMA,GPIO_CAMERA_LDO_SUB_VCAMA_M_GPIO);
			mt_set_gpio_dir(GPIO_CAMERA_LDO_SUB_VCAMA,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO_CAMERA_LDO_SUB_VCAMA,GPIO_OUT_ONE);
			#else
			if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))
            {
                PK_DBG("[CAMERA SENSOR] Fail to enable analog power (VCAM_A), power id = %d\n", CAMERA_POWER_VCAM_A);
			...
			printk("[jiacheng][jiacheng][jiacheng]---------------------------------------------\n");
			#endif             			
            mdelay(2);
           ...
           //enable active sensor
			//PWN 
			{
				if(mt_set_gpio_mode(CAMERA_CMPDN1_PIN,CAMERA_CMPDN1_PIN_M_GPIO)){PK_DBG("[kd_camera_hw] set gpio mode failed!! (CMPWN)\n");}
				if(mt_set_gpio_dir(CAMERA_CMPDN1_PIN,GPIO_DIR_OUT)){PK_DBG("[kd_camera_hw] set gpio dir failed!! (CMPWN)\n");}
				if(mt_set_gpio_out(CAMERA_CMPDN1_PIN,GPIO_OUT_ZERO)){PK_DBG("[kd_camera_hw] set gpio failed!! (CMPWN)\n");}
			}
			mdelay(2);
			//RST
			{
				if(mt_set_gpio_mode(CAMERA_CMRST1_PIN,CAMERA_CMRST1_PIN_M_GPIO)){PK_DBG("[kd_camera_hw] set gpio mode failed!! (CMRST)\n");}
				if(mt_set_gpio_dir(CAMERA_CMRST1_PIN,GPIO_DIR_OUT)){PK_DBG("[kd_camera_hw] set gpio dir failed!! (CMRST)\n");}
				if(mt_set_gpio_out(CAMERA_CMRST1_PIN,GPIO_OUT_ONE)){PK_DBG("[kd_camera_hw] set gpio failed!! (CMRST)\n");}
			}
			mdelay(2);
        } 
        ...
        if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW, currSensorName)) && (pinSetIdx == 0))
        {
			PK_DBG("power off  get in---  SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW \n");
		...
		/*oujiacheng@wind-mobi.com 20150819 begin*/
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW_GEER, currSensorName)) && (pinSetIdx == 0))
		/*oujiacheng@wind-mobi.com 20150819 end*/
        {
			PK_DBG("power off  get in---  SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW \n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
         ...
         {
        	PK_DBG("power off  get in---  SENSOR_DRVNAME_GC2355_MIPI_RAW \n");
		  //Set PWN low
		    {
			   if(mt_set_gpio_mode(CAMERA_CMPDN1_PIN,CAMERA_CMPDN1_PIN_M_GPIO)){PK_DBG("[kd_camera_hw] set gpio mode failed!! (CMPDN)\n");}
			   if(mt_set_gpio_dir(CAMERA_CMPDN1_PIN,GPIO_DIR_OUT)){PK_DBG("[kd_camera_hw] set gpio dir failed!! (CMPDN)\n");}
			   if(mt_set_gpio_out(CAMERA_CMPDN1_PIN,GPIO_OUT_ONE)){PK_DBG("[kd_camera_hw] set gpio failed!! (CMPDN)\n");}
		    }
			//Set Reset Pin Low
		    {
               if(mt_set_gpio_mode(CAMERA_CMRST1_PIN,CAMERA_CMRST1_PIN_M_GPIO)){PK_DBG("[kd_camera_hw] set gpio mode failed!! (CMRST)\n");}
               if(mt_set_gpio_dir(CAMERA_CMRST1_PIN,GPIO_DIR_OUT)){PK_DBG("[kd_camera_hw] set gpio dir failed!! (CMRST)\n");}
               if(mt_set_gpio_out(CAMERA_CMRST1_PIN,GPIO_OUT_ZERO)){PK_DBG("[kd_camera_hw] set gpio failed!! (CMRST)\n");}	   
            ...
            //VCAM_A
             ...
             mt_set_gpio_mode(GPIO_CAMERA_LDO_SUB_VCAMA,GPIO_CAMERA_LDO_SUB_VCAMA_M_GPIO);
			 mt_set_gpio_dir(GPIO_CAMERA_LDO_SUB_VCAMA,GPIO_DIR_OUT);
			 mt_set_gpio_out(GPIO_CAMERA_LDO_SUB_VCAMA,GPIO_OUT_ZERO);
			 #else
			  if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A,mode_name)) {
                PK_DBG("[CAMERA SENSOR] Fail to OFF analog power (VCAM_A), power id= (%d) \n", CAMERA_POWER_VCAM_A);
                //return -EIO;
			...
			//DVDD VGP3
             mt_set_gpio_mode(GPIO_CAMERA_LDO_SUB_VCAMD,GPIO_CAMERA_LDO_SUB_VCAMD_M_GPIO);
             mt_set_gpio_dir(GPIO_CAMERA_LDO_SUB_VCAMD,GPIO_DIR_OUT);
             mt_set_gpio_out(GPIO_CAMERA_LDO_SUB_VCAMD,GPIO_OUT_ZERO);
            mdelay(1);
         ...
         {
        	PK_DBG("power off  get in---  SENSOR_DRVNAME_OV2680_MIPI_RAW \n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
...
/*oujiacheng@wind-mobi.com 2015.5.22 add end*/
    return 0;
