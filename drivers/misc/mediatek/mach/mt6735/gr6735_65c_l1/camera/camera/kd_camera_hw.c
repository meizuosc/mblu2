...
u32 pinSet[3][8] = {
    ...
    if (On) {
        ...
        if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_S5K3L2_MIPI_RAW, currSensorName)) || (0 == strcmp(SENSOR_DRVNAME_S5K3L2_MIPI_RAW_GEER, currSensorName))))
        {
        ...
        else if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW, currSensorName)) || (0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW_GEER, currSensorName))))
         {
           ...
           //enable active sensor
			//PWN 
			{
				...
				if(mt_set_gpio_out(CAMERA_CMPDN1_PIN,GPIO_OUT_ONE)){PK_DBG("[kd_camera_hw] set gpio failed!! (CMPWN)\n");}
			}
			mdelay(2);
			//RST
			{
        ...
        if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_S5K3L2_MIPI_RAW, currSensorName)) || (0 == strcmp(SENSOR_DRVNAME_S5K3L2_MIPI_RAW_GEER, currSensorName))))
        {
         ...
         {
        	PK_DBG("power off  get in---  SENSOR_DRVNAME_OV5670_MIPI_RAW \n");
            //Set PWN low
		   {
			   ...
			   if(mt_set_gpio_out(CAMERA_CMPDN1_PIN,GPIO_OUT_ZERO)){PK_DBG("[kd_camera_hw] set gpio failed!! (CMPDN)\n");}
		   }
