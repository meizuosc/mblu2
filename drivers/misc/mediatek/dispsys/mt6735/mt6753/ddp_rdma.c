...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle, enum RDMA_MODE mode, bool isidle)
{
  ...
  if (idx == 0)
  {
      ...
      DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0xFF);//0xff
      ...
      if (width > 800)// FHD
      {
		  ...
		  else
          {
             if (isidle)
			 {
			 	sodi_threshold = 84|(158<<10);//low:84	high:158 (decouple in FHD in idle)
